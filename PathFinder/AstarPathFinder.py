...
pygame.display.set_caption("A* Pathfinding Alg")
...
def algorithm(draw, grid, start, end):
    ...
    while not q.empty():
        ...
        for n in curr.neighbors:
            temp_gS = g_score[curr] + 1
            n.make_closed()

            if temp_gS < g_score[n]:
                came_from[n] = curr
                g_score[n] = temp_gS
                f_score[n] = temp_gS + h(n.get_pos(), end.get_pos())
                count+=1

                q.put((f_score[n], count, n))
                n.make_open()
